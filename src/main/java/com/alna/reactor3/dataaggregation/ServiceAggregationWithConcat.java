...
@Slf4j
public class ServiceAggregationWithConcat {
  public static void main(String[] args) {
    ...
    try (NitriteTestDatabase testDatabase = new NitriteTestDatabase(Optional.of(schema))) {
      ...
      final Flux<Product> ownedProductList = CustomerProductPurchaseHistoryDataAccess.selectOwnedProducts(
          testDatabase.getNitriteDatabase(), schema.Customer1UUID).subscribeOn(Schedulers.elastic()).log();
      ...
      //When subscribed to the concat flux it subscribes to the underlying sources sequentially one after another.
      final Flux<Object> customerAggregateStream =
          Flux.concat(customerFlux.cast(Object.class),
